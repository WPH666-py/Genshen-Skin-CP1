// 原神 CP 壁纸套件1 — 米提亚 × 沃雅妮莎 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-Skin-CP1.git';
const EXT_ID = 'wp666.genshin-skin-cp1';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshincp1').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshincp1').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-Skin-CP1'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-Skin-CP1') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshin_skin_cp1', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshin_skin_cp1.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshin_skin_cp1.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshin-skin-cp1' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-Skin-CP1') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshin_skin_cp1.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
const CARDS = [
  { mode: 'single1', t: '第1张 · 比心', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'single2', t: '第2张 · 共舞', img: 'thumb-single2.png', d: '模糊背景 + 居中卡片' },
  { mode: 'single3', t: '第3张 · 道歉', img: 'thumb-single3.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '比心 · 满屏', img: 'thumb-cover1.png', d: 'cover 裁切铺满' },
  { mode: 'cover2', t: '共舞 · 满屏', img: 'thumb-cover2.png', d: 'cover 裁切铺满' },
  { mode: 'cover3', t: '道歉 · 满屏', img: 'thumb-cover3.png', d: 'cover 裁切铺满' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件1 · 米提亚 × 沃雅妮莎 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshincp1.set1', () => runCli(['1'], '已切换: 比心'));
  reg('genshincp1.set2', () => runCli(['2'], '已切换: 共舞'));
  reg('genshincp1.set3', () => runCli(['3'], '已切换: 道歉'));
  reg('genshincp1.setRandom', () => runCli(['random'], '已随机换一张!'));
  reg('genshincp1.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshincp1.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshincp1.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshincp1.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP1 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshincp1.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshincp1')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshincp1.gallery', new GalleryProvider(), {
