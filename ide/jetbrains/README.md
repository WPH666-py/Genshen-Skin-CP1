# JetBrains 系 IDE (PyCharm / WebStorm / IntelliJ / GoLand) — 原神CP1 背景图
...
1. 生成全部素材:
   ...
   ```bash
   genshin-cp1 all --out "%USERPROFILE%\GenshinCP1-Backgrounds"   # Windows
   genshin-cp1 all --out ~/GenshinCP1-Backgrounds                 # macOS / Linux
   ```

   源码形态: `python -m genshin_skin_cp1.cli all --out 目录`
...
- 想换壁纸时重复第 3 步选另一张即可。
- 与桌面壁纸互不影响: 桌面用 `genshin-cp1 2`, IDE 背景图单独设。
