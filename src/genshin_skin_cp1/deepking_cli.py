# -*- coding: utf-8 -*-
"""
原神 CP 壁纸套件 1 —— DeepKing 皮肤命令行

  genshin-cp1 deepking              # 生成皮肤 JSON + 可视化预览, 并打印接入步骤
  genshin-cp1 deepking --check      # 校验仓库是否满足 DeepKing 转换契约
  genshin-cp1 deepking --what       # 只显示 DeepKing 会从仓库里提取到什么
  genshin-cp1 deepking --out DIR    # 指定输出目录
  genshin-cp1 deepking --css        # 额外导出「按 CSS 变量推导」的那一版(与 DeepKing 在线转换一致)
"""
import argparse
import json
import os
...
def main(argv=None):
    ...
    ap.add_argument("--out", default=None, help="输出目录(默认 ~/.genshin-cp1/deepking)")
