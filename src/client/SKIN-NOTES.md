...
本目录的 `genshin-cp1.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshin-cp1.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshin-cp1 deepking          # 导出 ~/.genshin-cp1/deepking/*.skin.json
  ```
...
```bash
genshin-cp1 deepking --check    # 应输出「通过」
genshin-cp1 deepking --what     # 看转换器实际会取到什么值
```
